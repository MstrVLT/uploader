...
namespace WebDocLoader.Controllers
{
    ...
    public class IllegalController : ControllerBase
    {
        ...
        public IllegalController(ILogger<IllegalController> logger, IServiceProvider provider)
        {
            _logger = logger;
            _provider = provider;
        }
            ...
            if (CheckIfXMLFile(data.file))
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        var msg = await Illegal.SaveIllegalRawXML(transaction, userIsn, data.file.FileName, stream);

                        if (msg.Success)
                        {
                            transaction.Commit();
                            return Ok(new {message = msg.Text, globalname = msg.GlobalName});
                        }
                        else
                        {
